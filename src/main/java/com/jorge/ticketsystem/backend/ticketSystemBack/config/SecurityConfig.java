...
public class SecurityConfig {
    ...
    @Bean
    // Para manejar el login. Este Bean te permitirá llamar al método .authenticate() en AuthController (el que procesa /api/v1/auth/login)
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http
            ...
            .sessionManagement(session ->
                session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))//Le prohíbe a Spring Security crear o guardar sesiones en el servidor 
            .authorizeHttpRequests(auth -> auth //Abre el bloque para empezar a dictar las reglas de qué endpoints requieren permisos y cuáles no
                ...
                // Exige que para ver, crear u operar con órdenes y tickets emitidos, el usuario debe estar autenticado
                // obligatoriamente con un token válido, sin importar el rol que tenga.
                .requestMatchers("/api/v1/orders/**").authenticated()
                .requestMatchers("/api/v1/issued-tickets/**").authenticated()

                .anyRequest().authenticated()//otra URL que se agregue en el futuro y no esté explícitamente listada arriba, requerirá por defecto estar autenticado
            )
            .authenticationProvider(authenticationProvider())//acopla a la seguridad el proveedor de datos de usuario con BCrypt
    ...
    @Bean
    //Este método previene los bloqueos de seguridad del navegador cuando el Frontend intenta consumir el Backend desde dominios/puertos distintos
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        //Para permitir credenciales, cookies o cabeceras de autenticación entre el frontend y el backend
